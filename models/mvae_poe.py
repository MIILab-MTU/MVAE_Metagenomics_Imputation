...
class MVAE_PoE(tf.keras.Model):
    ...
    def build(self, shape):
        ...
        if self.decoder_scalar_std:
            self.rPhi = [self.add_weight(shape=(1, 1), initializer=tf.initializers.Ones(), name=f"decoder_scalar_std_{s}") for s in range(self.num_of_views) ]
        else:
    ...
    def call(self, views):
        ...
        mean, var, logvar = self.average_encoders(views, views_id = list(np.arange(self.num_of_views)))
        #Kullback-Leibler 
        ...
        for i in range(self.num_of_z_sampled):
            ...
            views_sample = [ ( self.cdgn_decoders_mean[s](z_sample)) for s in range(self.num_of_views) ]    # decoder
            if self.decoder_scalar_std :
    ...
    def get_latents_space(self,views):
        ...
        all_latent_space.insert(0, self.average_encoders(views)[0].numpy()) # 0: average, Z
