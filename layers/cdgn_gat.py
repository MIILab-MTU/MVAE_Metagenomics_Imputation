...
class GraphAttention(tf.keras.layers.Layer):
    def __init__(self, 
        ...
        self.attn_ws = [tf.keras.layers.Dense(
            hidden_size) for _ in range(num_heads)]
        # output layer
    ...
    def call(self, inputs, debug=False):
        ...
        assert value.shape[
            1] == 1, f'second dim of value should be 1, but was {value.shape[1]}'
        assert query.shape[
            1] == self.max_neighbors, f'second dim of query should equal max_neighbors, but was {query.shape[1]}'
        ...
        for i in range(self.num_heads):
            ...
            mask = tf.sequence_mask(
                num_neighbors + 1, maxlen=self.max_neighbors)[:, tf.newaxis]
