...
class Cdgn_Krylov(tf.keras.Model):
    ...
    def build(self, input_shape):
        ...
        for layer in range(self.num_of_layer):
            if layer != 0 :
                self.krylovs.append(self.add_weight(shape=(self.hidden_layer_size*self.krylov_deep, self.hidden_layer_size),
                                                    trainable=True, name="cdgn_phi_kryl"))
                self.layers_dropout.append(tf.keras.layers.Dropout(1-self.keep_prob, name="cdgn_phi_kryl"))
            else:
                self.krylovs.append([])
                self.layers_dropout.append(tf.keras.layers.Dropout(1-self.keep_prob, name = "cdgn_phi_kryl"))
        self.krylovs[0] = self.add_weight(shape=(input_shape[0][0][1]*self.krylov_deep, self.hidden_layer_size), 
                                          trainable=True, name="cdgn_phi_kryl")
